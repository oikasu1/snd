...
;((root, factory) => {
  ...
  function toKasuUrls(tokens) {
    return (tokens || []).map((txt) => {
      // 偵測到空格，代表這是未對應的連字號語詞，拆成多個音檔
      if (txt.includes(' ')) {
        return txt.split(' ').map(p => `https://oikasu1.github.io/snd/mp3all/${p}.mp3`);
      }
...
const HAKKA_LANGS = new Set([
	...
	const parts =
        ...
        for (const part of parts) {
            ...
            if (RE.multi.test(sandhi)) {
              // 以前是 split 成多個，現在改成取代為空格，當作一個 Token
              tokens.push(sandhi.replace(/-+/g, " "))
  ...
  function zhaoanTokenizeBase(s) {
    ...
    const tokens = []
    for (const part of parts) {
      const base = part.replace(/_/g, "-").replace(/-+/g, "-").replace(/^-+|-+$/g, "").toLowerCase()
      if (!base) continue
  ...
  class SequentialAudioPlayer {
    ...
    preload(urlGroups) {
      // 攤平可能出現的巢狀陣列
      const flat = Array.isArray(urlGroups) ? urlGroups.flat(Infinity).filter(Boolean) : [];
      ;[...new Set(flat)].forEach((u) => {
        const a = new Audio(u)
        a.preload = "auto"
      })
    }
    ...
    play(urlGroups, iconEl) {
      if (!Array.isArray(urlGroups) || urlGroups.length === 0) return
      ...
      // 將傳入的 URL 群組轉為扁平隊列，並記錄每個 URL 對應的 UI Token 索引
      this._queue = []
      urlGroups.forEach((group, i) => {
        if (Array.isArray(group)) {
          group.forEach(u => {
            if(u) this._queue.push({ url: u, tokenIndex: i })
          })
        } else if (group) {
          this._queue.push({ url: group, tokenIndex: i })
        }
      })
    ...
    _next() {
      ...
      // 這裡很重要：我們發送 tokenIndex 給 UI，而不是網址！
      this._emit("playing", item.tokenIndex)
